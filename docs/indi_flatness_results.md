# flatness outer-loop controller — In-firmware INDI outer loop + flatness feedforward (results)
...
trajectory-server, closes the **lemniscate-tracking gap** that angular-acceleration feedback (inner-loop
only) left open — bringing the offboard flatness advantage into firmware, at
the low-latency controller rate.

## Battery position tracking (RMSE, m)

| case | stock (stock-guided) | offboard | legacy INDI rate controller | angular-acceleration feedback (inner) | **flatness outer loop** |
| ---------------- | ----- | ----- | ----- | ----- | --------- |
...
altitude controller — see below). Source: `indi_flatness_sitl.json`.

**Metric note.** The first four columns are XKF1-vs-trajectory position RMSE on the
trajectory clock (`align.evaluate_bin`). flatness outer loop is scored from the `INDB` .BIN
...
angular-acceleration feedback is an inner-loop replacement, so its position RMSE necessarily tracks stock —
on the lemniscates that is **5.0 / 1.7 m**, because the stock guided outer loop
does not feed acceleration/jerk/snap. offboard showed that a flatness outer loop pulls
this to **0.46 / 0.77 m**, but only offboard (SET_ATTITUDE_TARGET, where the INDI
*accel* loop was latency-unstable). flatness outer loop runs the whole flatness + linear-INDI
outer loop **in firmware** and reaches **0.51 / 0.71 m** — matching offboard, and
beating stock/angular-acceleration feedback by ~10× (slow) and ~2.4× (fast). The in-firmware low-latency
path works where offboard did not.
...
- **The drag-rejection benefit is realistic-physics.** Tracking is insensitive to
  ...
  Its disturbance-rejection payoff needs realistic-physics's aero fidelity; in SITL, flatness outer loop's win
...
- **Attitude-only (`CC3_B_THR_EN=0`).** The outer loop drives the attitude target
  ...
  (climb to ~66 m). It is disabled; the RPM-fed thrust path is deferred to HW/realistic-physics,
  consistent with the offboard finding that the INDI *thrust* loop is the fragile part.

- **The integration fix (why angular-acceleration feedback alone could not track).** The outer loop first
