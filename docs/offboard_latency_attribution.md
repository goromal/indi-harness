# offboard vs stock-guided: latency attribution

Design-doc offboard exit gate: *"offboard controller flies the stock-guided trajectory set
stably; degradation vs stock guided mode understood and attributable to
latency."* This documents the stock-guided↔offboard comparison, the measured command-path
...
```
nix-build pkgs/nixos/sitl-envs/offboard-flatness-tracking.nix   # from anixpkgs
```

## Per-case RMSE (m)

Two independent fresh-VM offboard runs (run-1, run-2); stock-guided from
`baselines/stock_guided_sitl.json`.

| case            |    stock-guided |  offboard run-1 |  offboard run-2 | offboard avg |  Δ (offboard−stock-guided) |  Δ %   |
|-----------------|------:|----------:|----------:|-------:|-----------:|-------:|
...
per pose . So the offboard loop closes at ~27.7 Hz with a
...
cadence, versus stock-guided's stock controller running the full guided
...
inner path (onboard)." An offline study (`tests/test_offboard_bridge.py::
...
INDI** (ArduPilot firmware, design-doc onboard+): the same synchronization
...
**Is the offboard↔stock-guided delta attributable to latency?** Partly, and the honest
reading is more interesting than "uniform degradation":

- offboard's RMSE is remarkably **flat** across trajectories (0.45–0.79 m) while stock-guided
  ...
  it sees state.
- That floor is a **large win** on the aggressive trajectories, where stock-guided's
  ...
  exact attitude/thrust for the reference, so offboard tracks tightly regardless of
  how hard the trajectory is.
- That same floor is a **regression** on the gentle trajectories, where stock-guided is
  already well below it (circle_slow 0.27→0.76, +179%; circle_fast 0.64→0.78,
  +22%). offboard cannot beat a 0.27 m stock result from behind a 0.5–0.8 m
...
**control-strategy difference** — flatness feed-forward (offboard) vs stock guided
position tracking (stock-guided) — with the offboard command cadence contributing a
roughly constant ~0.5–0.8 m floor. The one unambiguous latency-attributable
degradation is circle_slow, where offboard hits that floor while stock-guided does not.

**Bottom line for the offboard exit.** The offboard ROS 2 outer loop flies the stock-guided
...
trajectories. Absolute-number parity with onboard control was never the offboard
goal ; consistent, stable, feed-forward-driven
...
increment — is carried to the onboard onboard work.
