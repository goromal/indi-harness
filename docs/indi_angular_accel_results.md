# angular-acceleration and RPM feedback (angular-acceleration feedback) results — in-firmware INDI, roll/pitch ω̇-inversion

**What shipped:** an in-firmware quaternion INDI attitude/rate backend
(`AC_CustomControl_INDI`, `CC_TYPE=3`) that flies the stock-guided trajectory battery in
SITL with a **clean rate loop** (no limit cycle) and **real angular-acceleration
(ω̇) inversion on roll and pitch**. This is the "angular-acceleration feedback" slice of actuator feedback. The
bidi-DShot payload originally scoped for actuator feedback (RPM-fed allocation, the G2
rotor-inertia term, the per-motor speed loop) is **deferred to hardware / realistic-physics** —
see "Why the RPM work is deferred" below.

Validated headless in the `indi-angular-accel-inversion.nix` NixOS VM gate (fork `goromal/ardupilot`
`dev/controller` @ `cba0b1b0`, `indi-harness` `master` @ `d356c00`).

## Battery RMSE (XKF1 position tracking, m)

| case | stock (stock-guided) | offboard | legacy INDI rate controller | **angular-acceleration feedback** |
|------|-----------:|------------:|--------:|-----------------:|
...
**Read this correctly.** angular-acceleration feedback is an *inner-loop* replacement that keeps the stock
guided position→attitude outer loop (identical command path to stock-guided). So the
trajectory-tracking RMSE necessarily tracks **stock** — angular-acceleration feedback matches it case-for-case
...
beat the offboard prototype on the lemniscates: offboard's advantage there comes from
its flatness feedforward in the *outer* loop, which is flatness outer loop work, not done here.

The position-RMSE battery therefore is **not** where angular-acceleration feedback's improvement shows up — a
clean inner loop and a limit-cycling inner loop can fly the same trajectory to the
same position RMSE (the EKF/outer loop absorbs inner-loop buzz). angular-acceleration feedback's improvement is
at the rate-loop level, below what position RMSE sees:

## The angular-acceleration feedback result: real ω̇-inversion, no limit cycle
...
**This is the milestone.** legacy INDI rate controller shipped a *de-tuned* INDI: to avoid a motor-buzz
...
integral action, not inversion). angular-acceleration feedback removes that compromise.

### How: the limit cycle was delay-driven, not gain-driven

Diagnosed via a cutoff × G1 SITL sweep. legacy INDI rate controller's angular-accel estimate was
...
|---|---|---|---|
| 30–40 (legacy INDI rate controller) | 1000 | 20–36 deg/s (limit cycle) | ~1.1 (not tracking) |
| **80 (angular-acceleration feedback)** | **500** | **3–4 deg/s (clean)** | **0.48 / 0.49 (tracking)** |

angular-acceleration feedback raises the estimator cutoff to 80 Hz (`filter-then-differentiate`, added as
...
needs the **G2 rotor-inertia term fed by measured RPM** . SITL does
...
## Why the RPM work (true actuator feedback) is deferred to HW / realistic-physics

While wiring the measured-RPM path we established that **SITL does not model the
physics actuator feedback's RPM mechanisms address**:
...
per-motor RPM loop cannot be validated in SITL — this is the realistic-physics fidelity wall
...
wired into the control loop. The design doc's A→C→B→realistic-physics order is, in effect, revised:
actuator feedback's bidi-DShot payload needs realistic-physics-level fidelity (or real hardware) to validate.

## Deferred to hardware / realistic-physics
...
- flatness outer-loop + flatness feedforward (the offboard lemniscate advantage).
...
nix-build pkgs/nixos/sitl-envs/indi-angular-accel-inversion.nix -o /tmp/angular-accel-gate
...
`indi-harness/baselines/indi_angular_accel_sitl.json`.
